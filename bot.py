...
def health():
    return "✅ Mass Kick Bot is running!", 200
...
# ============================================================
# MASS KICK FUNCTION (BAN NAHI - SIRF KICK)
# ============================================================
async def mass_kick_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not bot_member.can_restrict_members:
            await update.message.reply_text(
                format_with_emojis("𝐁𝐨𝐭 𝐝𝐨𝐞𝐬𝐧'𝐭 𝐡𝐚𝐯𝐞 𝐤𝐢𝐜𝐤 𝐩𝐞𝐫𝐦𝐢𝐬𝐬𝐢𝐨𝐧!"),
                parse_mode="HTML"
            )
            return
    ...
    status_msg = await update.message.reply_text(
        format_with_emojis("🔥 𝐌𝐀𝐒𝐒 𝐊𝐈𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃..."),
        parse_mode="HTML"
    )
    ...
    try:
        ...
        for i, member_id in enumerate(members):
            try:
                ...
                # ✅ TURANT UNBAN KARO (TAKI WAPAS AA SAKE)
                await context.bot.unban_chat_member(chat_id, member_id)
                kicked_count += 1
            except Exception as e:
                failed_count += 1
                logger.error(f"Failed to kick {member_id}: {e}")
            
            # Show progress every 10 kicks
            if (i + 1) % 10 == 0 or (i + 1) == total_members:
                progress = int((i + 1) / total_members * 100)
                elapsed = (datetime.now() - start_time).seconds
                await status_msg.edit_text(
                    format_with_emojis(f"""🔥 𝐌𝐀𝐒𝐒 𝐊𝐈𝐂𝐊 𝐈𝐍 𝐏𝐑𝐎𝐆𝐑𝐄𝐒𝐒

📊 𝐆𝐫𝐨𝐮𝐩: {stylish(chat.title)}
👥 𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬: {stylish(str(kicked_count))}/{stylish(str(total_members))} ({stylish(str(progress))}%)
✅ 𝐊𝐢𝐜𝐤𝐞𝐝: {stylish(str(kicked_count))}
...
⏱️ 𝐄𝐥𝐚𝐩𝐬𝐞𝐝: {stylish(str(elapsed))}𝐬"""),
        ...
        elapsed = (datetime.now() - start_time).seconds
        final_msg = f"""✅ 𝐌𝐀𝐒𝐒 𝐊𝐈𝐂𝐊 𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐄𝐃
...
{e('warning')} 𝐀𝐥𝐥 𝐦𝐞𝐦𝐛𝐞𝐫𝐬 𝐡𝐚𝐯𝐞 𝐛𝐞𝐞𝐧 𝐤𝐢𝐜𝐤𝐞𝐝! {e('warning')}"""
...
# /masskick COMMAND
# ============================================================
async def masskick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await mass_kick_group(update, context)
...
𝐂𝐨𝐦𝐦𝐚𝐧𝐝𝐬:
/𝐦𝐚𝐬𝐬𝐤𝐢𝐜𝐤 - 𝐊𝐢𝐜𝐤 𝐚𝐥𝐥 𝐦𝐞𝐦𝐛𝐞𝐫𝐬

⚠️ 𝐎𝐧𝐥𝐲 𝐨𝐰𝐧𝐞𝐫 𝐜𝐚𝐧 𝐮𝐬𝐞!
⚠️ 𝐁𝐨𝐭 𝐦𝐮𝐬𝐭 𝐛𝐞 𝐚𝐝𝐦𝐢𝐧!
⚠️ 𝐒𝐢𝐫𝐟 𝐤𝐢𝐜𝐤 - 𝐁𝐚𝐧 𝐧𝐚𝐡𝐢!"""
...
def main():
    ...
    application.add_handler(CommandHandler("masskick", masskick))
    
    logger.info("Mass Kick Bot Started!")
