...
import json
import os
import random  # ✅ YAHAN IMPORT KIYA
import string
import asyncio
...
async def banall(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if bot_member.status not in ['administrator', 'creator']:
            await update.message.reply_text(
                format_with_emojis("❌ 𝐁𝐨𝐭 𝐢𝐬 𝐧𝐨𝐭 𝐚𝐧 𝐚𝐝𝐦𝐢𝐧!"),
                parse_mode="HTML"
            )
            return
        ...
        if not bot_member.can_restrict_members:
            await update.message.reply_text(
                format_with_emojis("❌ 𝐍𝐨 𝐛𝐚𝐧 𝐩𝐞𝐫𝐦𝐢𝐬𝐬𝐢𝐨𝐧!"),
                parse_mode="HTML"
            )
            return
    ...
    if not to_ban:
        await update.message.reply_text(
            format_with_emojis("📭 𝐍𝐨 𝐬𝐚𝐯𝐞𝐝 𝐮𝐬𝐞𝐫𝐬 𝐢𝐧 𝐭𝐡𝐢𝐬 𝐠𝐫𝐨𝐮𝐩!"),
            parse_mode="HTML"
        )
        return
